<?php
/*
Plugin Name: JR Alexa
Plugin URI: http://www.jakeruston.co.uk/2010/04/wordpress-plugin-jr-alexa/
Description: Allows you to show your alexa rank as a widget on your blog!
Version: 1.1.1
Author: Jake Ruston
...
add_action('admin_menu', 'jr_Alexa_add_pages');

// action function for above hook
function jr_Alexa_add_pages() {
    add_options_page('JR Alexa', 'JR Alexa', 'administrator', 'jr_Alexa', 'jr_Alexa_options_page');
}
...
register_activation_hook(__FILE__,'alexa_choice');

if (!function_exists("jr_Alexa_refresh")) {
function jr_Alexa_refresh() {
update_option("jr_submitted_Alexa", "0");
}
}

function alexa_choice () {
if (get_option("jr_Alexa_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_Alexa";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Alexa", "1");
wp_schedule_single_event(time()+172800, 'jr_Alexa_refresh'); 
} else {
...
update_option("jr_Alexa_links_choice", $content);
}
}

if (get_option("jr_alexa_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_alexa_link_personal", $content);
}
}

// jr_Alexa_options_page() displays the page content for the Test Options submenu
function jr_Alexa_options_page() {

    // variables for the field and option names
    $opt_name_1 = 'mt_Alexa_ID';	
    $opt_name_5 = 'mt_Alexa_plugin_support';
    $hidden_field_name = 'mt_Alexa_submit_hidden';
	$data_field_name_1 = 'mt_Alexa_ID';
    $data_field_name_5 = 'mt_Alexa_plugin_support';
...
<?php
    ...
    echo "<h2>" . __( 'JR Alexa Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
<?php
    ...
    $change3 = get_option("mt_Alexa_plugin_support");
...
function init_Alexa_widget() {
register_sidebar_widget('JR Alexa', 'Alexa_init');
}

function Alexa_init($args) {
extract($args);

$title=get_option("mt_Alexa_ID");

if ($title=="") {
$title="Alexa";
}

$blog_url_feedback23=get_bloginfo('url');
$blog_url_feedback23=str_replace("http://", "", $blog_url_feedback23);

echo $before_widget.$before_title.$title.$after_title."<br />";
?>
<A href="http://www.alexa.com/siteinfo/<?php echo $blog_url_feedback23; ?>"><SCRIPT type='text/javascript' language='JavaScript' src='http://xslt.alexa.com/site_stats/js/t/a?url=<?php echo $blog_url_feedback23; ?>'></SCRIPT></A>
<?php
echo $after_widget;
$supportplugin=get_option("mt_Alexa_plugin_support");
if ($supportplugin=="" || $supportplugin=="Yes") {
add_action('wp_footer', 'Alexa_footer_plugin_support');
}
}

function Alexa_footer_plugin_support() {
$linkper=utf8_decode(get_option('jr_alexa_link_personal'));

if (get_option("jr_Alexa_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_Alexa_links_choice'));
...
update_option("jr_Alexa_links_choice", $new);
update_option("jr_Alexa_link_newcheck", "444");
}

if (get_option("jr_submitted_Alexa")=="0") {
$pname="jr_Alexa";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Alexa", "1");
...
wp_schedule_single_event(time()+172800, 'jr_Alexa_refresh'); 
} else if (get_option("jr_submitted_Alexa")=="") {
$pname="jr_Alexa";
$url=get_bloginfo('url');
$current=get_option("jr_Alexa_links_choice");
...
wp_schedule_single_event(time()+172800, 'jr_Alexa_refresh'); 
  ...
  $pshow = "<p style='font-size:x-small'>Alexa Plugin created by ".$linkper." - ".stripslashes(get_option('jr_Alexa_links_choice'))."</p>";
  echo $pshow;
}

add_action("plugins_loaded", "init_Alexa_widget");
